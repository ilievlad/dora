...
@app.route("/jenkins-parser", methods=["POST"])
def index():
    """
    ...
    try:
        ...
        if "X-Jenkins-Token" in attr:
            event = process_jenkins_event(attr, msg, msg_id)
...
def process_jenkins_event(headers, msg, msg_id):
    event_type = "deployment"
    signature = headers["X-Jenkins-Signature"]["Value"]
    source = "jenkins"
    ...
    time_created = metadata["buildTimestamp"]
    e_id = metadata["buildTag"]
